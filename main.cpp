...
int main()
{
    ...
    get_pos_xy(60,30);
    printf("At the end of the demonstration, press any key to exit\n");
    getch();
}
...
void InputMaxTime()
{
    printf("\n\t\t\t Please enter the maximum running time of the elevator. The elevator system will stop automatically when this time is reached\n");
    printf("\t\t\t please input:");
    while (scanf("%d", &MaxRunTime) != 1 || MaxRunTime <= 500 || MaxRunTime > 10000)
    {
        printf("\t\t\t input error, please try it again!（500=<x<10000）\n");
        while (getchar() != '\n')
            continue;
        printf("\t\t\tplease input:");
    }
